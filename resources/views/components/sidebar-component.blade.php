...
<nav id="sidebar" class="sidebar">
    ...
    <div class="sidebar-header update_sidebar">
        ...
        @if (!is_null($school_config->logo))
            ...
            <img id="sidebarlogo" src="{{ asset('public/uploads/settings/logo-jiliu-sidebar.svg') }}" alt="logo" style="display:block;">
            <img id="sidebarlogo-short" src="{{ asset('public/uploads/settings/logo-jiliu-sidebar-simple.svg') }}" alt="logo" style="display:none;">
        @else
    ...
    @if (Auth::user()->is_saas == 0)
        <ul class="sidebar_menu list-unstyled" id="sidebar_menu">
            ...
            @else
                @if (auth()->user()->role_id != 3 && auth()->user()->role_id != 10)
                    @isset($sidebar_menus)
                        ...
                        @foreach ($sidebar_menus as $sidebar_menu)
                             @if($sidebar_menu->subModule->count() > 0 && sidebarPermission($sidebar_menu->permissionInfo)==true)
                                @if ($sidebar_menu->permissionInfo->name)
                                    ...
                                    <span class="menu_seperator" id="seperator_{{ $sidebar_menu->permissionInfo->route }}" data-section="{{ $sidebar_menu->permissionInfo->route }}">
										@if($sidebar_menu->permissionInfo->name)
                                            @switch($sidebar_menu->permissionInfo->name)
                                                @case("Dashboard")
                                                    Panel
                                                    @break
                                                @case("Administration") 
												...
												@case("Module")
                                                    ...
                                                    @break
                                                @default
                                                    {{ $sidebar_menu->permissionInfo->name }}
                                            @endswitch                                        
										@endif
			</span>
                                @endif
