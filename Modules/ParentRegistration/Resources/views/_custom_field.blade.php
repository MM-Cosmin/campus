@if (isset($custom_fields))
    ...
    @foreach ($custom_fields as $key => $custom_field)
        @if ($custom_field->type == 'textInput')
            <div class="{{ $custom_field->width }} input-control">
                <div>
                    ...
                    @endif
                    <input class="input-control-input" type="text" name="customF[{{ $custom_field->label }}]"
        ...
        @elseif($custom_field->type == 'numericInput')
            ...
            <div class="{{ $custom_field->width }} input-control">
                <div>
                    ...
                    @endif
                    <input class="input-control-input" type="number" min="{{ $min_max_value[0] }}"
                        max="{{ $min_max_value[1] }}" name="customF[{{ $custom_field->label }}]"
                        value="{{ isset($student) ? customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name) : '' }}">
                    {{-- <label>{{$custom_field->label}} <span>{{($custom_field->required==1)? "*":""}}</span> </label> --}}
                    @if ($errors->has($custom_field->label))
                        <span class="text-danger">
        ...
        @elseif($custom_field->type == 'multilineInput')
            ...
            <div class="{{ $custom_field->width }} input-control">
                <div>
                    ...
                    @endif
                    <textarea class="primary_input_field form-control" placeholder="{{ $custom_field->label }}" cols="0"
                        rows="3" name="customF[{{ $custom_field->label }}]">{{ isset($student) ? customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name) : '' }}</textarea>
            ...
            @if (activeTheme() == 'edulia')
                <div class="{{ $custom_field->width }}">
                    <div class="input-control">
                        <label for="datepicker4" class="input-control-label">{{ $custom_field->label }}
                            <span>{{ $custom_field->required == 1 ? ' *' : '' }}</span>
                        </label>
                        <input
            ...
            <div class="{{ $custom_field->width }} mt-30 d-flex align-items-center">
                ...
                @foreach ($checkbox_values as $key => $checkbox_value)
                    <div class="row no-gutters input-right-icon mr-3">
                        <div class="primary_input">
                            <input type="checkbox" id="custom_types_{{ $key }}_{{ $custom_field->id }}"
                                class="common-checkbox exam-checkbox" name="customF[{{ $custom_field->label }}][]"
                                @if (isset($student)) @if (!is_null(customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name)))
                                            @if (in_array($checkbox_value, customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name)))
                                                checked @endif
                                @endif
                ...
                @foreach ($name_values as $key => $name_value)
                    <label for="{{ $key }}_custField_{{ $custom_field->id }}" class="page_radio">
                        <input type="radio" name="customF[{{ $custom_field->label }}]"
                            ...
                            @if (isset($student) &&
                                    $name_value == customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name)) checked @endif value="{{ $name_value }}"
            ...
            <div class="d-flex radio-btn-flex ml-30 mt-1">
                @foreach ($name_values as $key => $name_value)
                    <div class="mr-30">
                        ...
                        <label
                            for="{{ $key }}_custField_{{ $custom_field->id }}">{{ $name_value }}</label>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    @else
        <div class="{{ $custom_field->width }}">
            <div class="form-group input-group">
                <select
                    class="primary_select form-control{{ $errors->has($custom_field->label) ? ' is-invalid' : '' }}"
                    name="customF[{{ $custom_field->label }}]">
                    <option
                        data-display="{{ $custom_field->label }} @lang('common.select') {{ $custom_field->required == 1 ? '*' : '' }}"
                        value="">
                ...
                <div class="col-auto">
                    <button class="primary-btn-small-input" type="button">
                        ...
                        <input type="file" id="photo_{{ $key }}"
                            data-id="#placeholderPhoto_{{ $key }}" class="d-none cutom-photo"
                            value="{{ isset($student) ? customFieldValue($student->id, $custom_field->label, $student->custom_field_form_name) : '' }}"
                            name="customF[{{ $custom_field->label }}]"
                            @if (isset($student)) {{ ' ' }}
                                        @else
