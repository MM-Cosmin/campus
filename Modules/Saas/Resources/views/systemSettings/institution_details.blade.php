...
@section('title')
{{$school->school_name}} @lang('common.details')
@endsection
@section('mainContent')
...
$active_style = App\SmStyle::where("school_id", Auth::user()->school_id)->where('is_active', 1)->first();
 ...
 $setting = generalSetting();  if(!empty($setting->currency_symbol)){ $currency = $setting->currency_symbol; }else{ $currency = '$'; }   @endphp
            ...
            <div class="row mb-30">
                <div class="col-lg-12">
                    <div class="main-title">
                        <h3>@lang('saas::saas.welcome_to') <small>{{$school->school_name}}</small></h3>
                    </div>
                </div>
            </div>
            <div class="row">
