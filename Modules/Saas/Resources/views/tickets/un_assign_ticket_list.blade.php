...
@section('title')
    @lang('saas::saas.un_assign_ticket_list')
@endsection
@section('mainContent')
        ...
        <div class="container-fluid">
            <div class="row justify-content-between">
                ...
                <div class="bc-pages">
                    ...
                    <a href="#">@lang('saas::saas.un_assign_ticket')</a>
                </div>
            </div>
        </div>
    </section>
    <section class="admin-visitor-area up_admin_visitor">
        <div class="container-fluid p-0">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="">
                        {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'route' => ['admin.un_assign_ticket_list.search'], 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'search_studentA']) }}
                        <div class="row white-box">
                            <input type="hidden" name="url" id="url" value="{{ URL::to('/') }}">
                            <input type="hidden" name="type" value="un_assign">
                                ...
                                <select
                                    ...
                                    @foreach ($category as $item)
                                        <option value="{{ $item->id }}"
                                            {{ isset($category_id) ? ($category_id == $item->id ? 'selected' : null) : null }}>
                                            {{ $item->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-4 mt-30-md">
                                <select
                                    ...
                                    @foreach ($priority as $item)
                                        <option value="{{ $item->id }}"
                                            {{ isset($priority_id) ? ($priority_id == $item->id ? 'selected' : null) : null }}>
                                            {{ $item->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-4 mt-30-md">
                                <select
                                    ...
                                    id="select_class" name="active_status">
                                    <option data-display="@lang('common.status') " value="">@lang('saas::saas.select_status') </option>
                                    <option value="0"
                                        {{ isset($status_id) ? ($status_id == 0 ? 'selected' : null) : null }}>
                                        @lang('common.pending')</option>
                                    <option value="1"
                                        {{ isset($status_id) ? ($status_id == 1 ? 'selected' : null) : null }}>
                                        @lang('saas::saas.ongoing')</option>
                                    <option value="2"
                                        {{ isset($status_id) ? ($status_id == 2 ? 'selected' : null) : null }}>
                                        @lang('saas::saas.complete')</option>
                                    @if (Auth::user()->role_id == 1)
                                        <option value="3"
                                            {{ isset($status_id) ? ($status_id == 3 ? 'selected' : null) : null }}>
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 no-gutters">
                            <div class="main-title">
                                <h3 class="mb-0">@lang('saas::saas.un_assign_ticket_list')</h3>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                                ...
                                <table id="table_id" class="table" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>@lang('saas::saas.ticket_priority')</th>
                                            <th>@lang('saas::saas.user_agent')</th>
                                        ...
                                        @foreach ($ticket as $value)
                                            @if (Auth::user()->role_id != 2 && Auth::user()->role_id != 3)
                                                <tr>
                                                    ...
                                                    <td>{{ @$value->agent_user ? $value->agent_user->full_name : 'Not assign yet !' }}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <x-drop-down>
                                                            <a class="dropdown-item"
                                                                href="{{ route('admin.ticket_view', $value->id) }}">
                                                                @lang('common.view')</a>
                                                            @if (Auth::user()->is_administrator == 'yes' || Auth::user()->id == $value->created_by)
                                                                <a class="dropdown-item"
                                                                    href="{{ route('admin.ticket_edit', $value->id) }}">
                                                                    @lang('common.edit')</a>
                                                                <a class="deleteUrl dropdown-item"
                                                                    data-modal-size="modal-md" title="@lang('common.delete_ticket')"
                                                                    href="{{ route('admin.ticket_delete_view', $value->id) }}">
                                                                    @lang('common.delete')
                                                                </a>
                                                                <a class="assign_ticket_modal dropdown-item"
                                                                    data-modal-size="modal-md"
                                                                    data-ticket_id="{{ $value->id }}">
                                                                    @lang('common.Assign')
                                                                </a>
                                                            @endif
                                                        </x-drop-down>
                            ...
                            <div class="col-lg-12">
                                <div class="row">
                                    <div class="col-lg-12" id="staff_class_div">
                                        <select class="primary_select" name="assign_user" id="select_staff_parent">
                                            <option data-display="@lang('hr.select_staff') *" value="">@lang('hr.select_staff')
                                                *</option>
